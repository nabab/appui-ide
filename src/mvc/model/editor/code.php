...
if ($model->hasData('id_project')) {
  if (!$isHistory) {
    ...
    $files = $fs->getFiles($model->dataPath('appui-newide') . 'backup/' . $model->data['id_project'] . '/' . X::join($bytes, '/') . '/' .  $type);
    ...
    return [
      ...
      "title" => 'History',
      'files' => $res,
      'path' => $model->dataPath('appui-newide') . 'backup/' . $model->data['id_project'] . '/' . X::join($bytes, '/') . '/' .  $type
    ];
  }
}