...
if ( isset($model->inc->options) ){
  if ( !empty($model->data['code']) && !empty($model->data['tableRepository']) ){
    $repositories = $model->inc->options->full_options($model->inc->options->from_code($model->data['code'],'PATHS', 'ide', BBN_APPUI));
  ...
  //  $infos = json_encode($model->inc->options->option($model->data['id']));
    $infos = $model->inc->options->option($model->data['id']);
