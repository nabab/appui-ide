...
//function that defines whether the search is sensitive or non-sensitive
  ...
  */
  ...
  $all = \bbn\file\dir::get_files($path, true);
  if ( is_array($all) ){
    if ( !empty($model->data['component']) ){
      $comp = $model->data['component'];
      $all = array_filter( $all, function($p) use($comp){
        $file = $p;
        $p = \bbn\str::file_ext($p, true)[0];
        if ( $comp === $p ){
          return $file;
        }
      });
    ...
    if ( !empty($all) ){
      ...
      if ( basename($v) !== "cli"  ){
        if ( is_dir($v) ){
          if ( !empty($model->data['mvc']) && isset($model->data['searchFolder']) && !empty($model->data['searchFolder']) ){
            $comp = $v.$model->data['searchFolder'];
            //die(\bbn\x::dump($v, $comp));
          }
          else{
            $comp = $v;
          }
        //  die(\bbn\x::dump($all, $comp,$model->data['searchFolder'],$model->data['component']));
