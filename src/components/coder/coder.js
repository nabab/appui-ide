  ...
  return {
    ...
    methods: {
      ...
      getExtensions() {
        ...
        if (!cm.languageExtensions[appuiIdeCoderCp.modeCode[this.currentMode]]) {
        ...
        extensions.push(cm.languageExtensions[appuiIdeCoderCp.modeCode[this.currentMode]]);
      ...
      getFirstNode(node, context) {
        bbn.fn.log("NODE1", node, context.state.sliceDoc(node.from, node.to));
      ...
      getComponentObject() {
        try {
          ...
          bbn.fn.log("IMPOSSIBLE TO EVALUATE OBJECT");
      ...
      getJavascriptCompletion(context, node) {
        ...
        if (false && first && first.startsWith('this')) {
          ...
          if (first === "this") {
            ...
            try {
              let prop = this.getNestedProp(cpObj, first)
      ...
      jsCompletionSource(context) {
        bbn.fn.log("jsCompletionSource", context)
