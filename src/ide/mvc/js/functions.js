if ( appui.f.IDE === undefined ){
  appui.f.IDE = {
    ...
    close: function(ele, cfg, idx){
      var conf = false;
      $(".ui-codemirror", ele).each(function(){
    ...
    view: function(d){
      ...
      if ( d.code && d.file ){
        ...
        $("div.k-content.k-state-active div.code:visible", appui.f.IDE.tabstrip).codemirror({value:d.code,mode:m}).scrollTop(0);
      }
      else if ( d.st ){
      ...
      if ( a.url ){
        ...
        if ( a.cfg !== undefined ){
          b.callback = function(ele){
            $(ele).find("div.code").codemirror("refresh");
          };
        }
        return b;
      }
